...
void setCoeffs() {
	...
	for (int i=0; i<n; i++) g1[i] = g2[i] = g3[i] = inter[i] = 0;
	...
	for (int i=1; i<35; i++) {
		...
		if (inter[i] == 1 || inter[i] == -1) {
			...
			if (F[i-k1] == -1 && inter[i] == -1) { //forced case
				...
				if (inter[i+35] <= -1 || inter[i+70] >= 1) return 0;
				//checks to make sure the terms are alternating
				if (F[i+35-k2] == -1 || F[i+70-k3] == 1) return 0;
				g1[i+35] = -1; g1[i+70] = 1;
				F[i+35] = g1[i+35] + g2[i+35] + g3[i+35];
				F[i+70] = g1[i+70] + g2[i+70] + g3[i+70];
			}
...
int main() {
	...
	f3[0] = f3[7] = f3[13] = f3[14] = f3[17] = f3[19] = 1;
