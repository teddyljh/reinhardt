...
int determine(int i) {
	//4 MUTUALLY EXCLUSIVE CASES FOR inter[i]
	...
	cout << "i: " << i << "  inter[i]: " << inter[i] << endl;

	if (inter[i] == 2) { 
		if (F[i-k1] == 1) return 0; //need to maintain the sign for i 
		else if (inter[i+35] +1 == F[i+35-k2]) return 0; //need to maintain the sign for i+35
		else if (inter[i+70] -1 == F[i+70-k3] ) return 0; // need to maintain the sign for i+70
		else if ((inter[i+35] + 1 > 1) || ((inter[b] -1) < -1)) return 0; //coefficients for F
		else {
			set(i, -1, 1, -1);
			determine(i+1);
		}
		...
		if (F[i-k1] == -1) return 0; //need to maintain the sign for i
		else if (inter[a] -1 == F[c]) return 0; //need to maintain the sign for i+35
		else if (inter[b] + 1 == F[d]) return 0; //need to maintain the sign for i+70
		else if ((inter[a] -1 < -1) || (inter[b] +1 > 1)) return 0; //coefficients too big
		else {
			set(i, 1, -1, 1);
			determine(i+1);
		}
...
int main() {
	...
	if (!determine(1)) cout << "failed explicit cases" << endl;
	cout << endl;
	printAns();
