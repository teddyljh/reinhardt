...
#which permutates all possible g1 with randomization of g2 and g3

import numpy as np
import random
#from convert_poly import *
...
def findF(rn,a_list, outfile):
    n = len(rn)
    
    A = np.zeros((pq, n), np.int32)
    B = np.zeros((pl, n), np.int32)
    C = np.zeros((ql, n), np.int32)
    
    g1= np.zeros((pql, n), np.int32)
    g2 = np.zeros((pql, n), np.int32)
    g3 = np.zeros((pql, n), np.int32)
    g12 = np.zeros((pql, n), np.int32) #g1+g2
    ...
    for i in range(1, pq+1):
        ...
        A[i-1] =findA(rn, i, a_list[i-1]) 
        
    g1 = repeat(A, l)
    
    for i in range(1, pl+1):
        B[i-1] = findB(rn, i, random.randint(0,1))
    ...
    for i in range(2, ql+1):
        C[i-1] = findC(rn, i, random.randint(0,1), C[i-2][n-1])
    ...
    if check_valid(F) == True and check_alt(F) == True and check_period(print_gaps(F)) == False: 
        ...
        print "g1:", convert_sign(reduce_dim(g1))
        print "g2:", convert_sign(reduce_dim(g2))
        print "g3:", convert_sign(reduce_dim(g3))
...
def findC(rn, i, m, former): 
    ...
    if i%35 == 1:
        c.append(0) #for F to start with 1
    else:
        c.append(-former)
    
    c.append(0)
    
    if m == 0 or i%35 == 0: #since C[0][0] is 0, the final is 0
        c.append(0)
...
def main():
    ...
    a_list = np.zeros(15, np.int32)
    for i in range (pq):
        for j in range(i):
            a_list[j] = 1
        
        permutate(a_list, 0, pq, 0)
    
    #print len(perm_a)
    #print perm_a
    
    out = open("data315.txt", "w")
 

    #for i in range(50):  #test with the first 50
    for i in range(len(perm_a)):
        findF(rn, perm_a[i], out)
