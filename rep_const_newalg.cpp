...
const int n = p*l*q*r;
int g1[n], g2[n], g3[n], g4[n], f2[15], f3[21], f1[35],  F[n], inter[n], repf2[5], repf3[7];
bool boolc[n][3];
...
void finalStep() {
	...
	for (int i=1; i<n; i++) {
		...
		else { //in this case, inter[i] = 0 or inter[i] = 2/-2 but that case has already been delt with 
			int k=1;
			while (F[i-k] == 0) {
				k++;
			}
